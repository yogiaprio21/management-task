...
import { Plus, Folder, Trash2, Layout, Calendar, ArrowRight, Loader2, X } from 'lucide-react';
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full max-w-lg">
              <Card className="p-8 md:p-10 shadow-2xl border-none">
                <div className="flex justify-between items-start mb-8">
                  ...
                  <button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-slate-100 rounded-xl"><X className="w-6 h-6 text-slate-400" /></button>
                </div>
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="flex gap-3 pt-4">
                    <Button type="submit" className="flex-1 h-12 text-base" isLoading={createMutation.isPending}>Create Project</Button>
                    <Button variant="secondary" type="button" onClick={() => setIsModalOpen(false)} className="flex-1 h-12 text-base">Cancel</Button>
                  </div>
                </form>
              </Card>
            </motion.div>
