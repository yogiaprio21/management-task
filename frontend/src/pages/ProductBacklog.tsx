        ...
        <div className="grid gap-3">
          {backlogLoading ? (
            ...
            backlogItems?.map((item) => (
              <Card key={item.id} className="py-4 px-6 flex items-center justify-between hover:border-primary transition-all">
                ...
                <div className="flex flex-col items-end gap-1">
                   <span className="text-[10px] uppercase font-bold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">{item.status}</span>
                </div>
              </Card>
            ))
          )}
