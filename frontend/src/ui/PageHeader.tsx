    ...
    <div className="min-w-0 space-y-2">
      <div className="flex min-w-0 items-center gap-3">
        ...
        <div className="min-w-0">
          <h1 className="truncate text-2xl font-bold tracking-tight text-slate-950 dark:text-slate-50 md:text-3xl">{title}</h1>
          {meta && <div className="mt-2 flex flex-wrap gap-2">{meta}</div>}
        </div>
      </div>
      {description && <p className="max-w-3xl text-sm leading-6 text-slate-600 dark:text-slate-300 md:text-base">{description}</p>}
    </div>
