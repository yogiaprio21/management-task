...
export class BacklogService {
  ...
  async create(itemData: Partial<BacklogItem>, user: User): Promise<BacklogItem> {
    if (!itemData.projectId) {
      throw new BadRequestException('Backlog item must be attached to a project');
    }
    const project = await this.backlogRepository.manager.getRepository('Project').findOne({
      where: { id: itemData.projectId },
      relations: ['members', 'workspace', 'workspace.members'],
    }) as any;
    ...
    if (!isProjectOwner && !isProjectMember && !isWorkspaceMember && user.role !== 'admin') {
      throw new ForbiddenException('You do not have access to this project');
    }
    await this.workspacesService.assertUserInWorkspace(project.workspaceId, itemData.assigneeId);
